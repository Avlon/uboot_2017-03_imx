...
#ifndef __MX6QDL_SECOA62_ANDROID_H
#define __MX6QDL_SECOA62_ANDROID_H
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"script_file=uEnvA62.txt\0" \
	"script=uEnv.txt\0" \
	"loadbootscript=" \
	         "ext2load mmc ${mmc_cur}:4 ${kernel_loadaddr} ${script};\0" \
	"bootscript=echo Running bootscript from mmc ...; " \
		"env import -t ${kernel_loadaddr} ${filesize};\0" \
	"usb_test=usb start; mw.b 10600000 0 1600; setenv recoveryboot 0;" \
		"if fatload usb 0 10600000 /boot/${script_file}; then " \
		"env import -t 10600000 1600; if test ${recoveryboot} = 1;" \
		"then boot; fi ; fi; \0" \
	"seco_boot_init=run usb_test;" \
